...
<template>
	<div class="flex flex-col w-screen items-center max-h-90vh relative z-1 ">
		<div v-if="question.meta && !question.meta.easy" class="flex items-center scale-90 sm:hidden w-100%"
			...
			<div class="timer bg-white lt-xxxl:h-55px! lt-xxxl:max-w-310px q-shadow-sm lt-xxxl:p-10px!">
				...
				<div class="time lt-xxxl:h-13px! lt-xxxl:w-90%! lt-xxxl:min-w-80%! bg-#D1EAD8!">
					<div v-if="question.type !== 'VALUE'" class="inner h-full bg-#71CB8A trs"
						:style="`width: ${100 - (100 * time) / totalTime}%`"></div>
					<div v-else class="inner h-full bg-#71CB8A trs"
						:style="`width: ${100 - (100 * time) / totalGameTime}%`"></div>
				</div>
				<!-- TODO: more font -->
		...
		<div v-if="question.meta && question.meta.easy" class="flex items-center scale-90 sm:hidden w-full"
			:class="{ 'pointer-events-none!': answerEmitting }">
			<div @click="goToStart"
		...
		<div class="card p-42px lt-xxxl:p-33px pb-48px w-770px lt-xxxl:w-610px lt-xxl:scale-90 lg:scale-100! scroll-y max-w-full lt-sm:-mt-20px"
			:class="[
				...
				{ 'fullvhcard': cardHaveImage }, { 'max-h-[calc(100vh-50px)]': !cardHaveImage }
			]">
			<div v-if="question.meta && !question.meta.easy" class="flex items-center gap-x-12px lt-sm:hidden">
				<div class="timer bg-white lt-xxxl:h-55px! lt-xxxl:max-w-310px q-shadow-sm lt-xxxl:p-10px!">
					<img class="w-24px h-24px lt-xxxl:w-18px lt-xxxl:h-18px" src="/img/clock.png" alt="таймер" />
					<div class="time lt-xxxl:h-13px! lt-xxxl:w-220px! lt-xxxl:min-w-220px! bg-#D1EAD8!">
						<div v-if="question.type !== 'VALUE'" class="inner h-full bg-#71CB8A trs"
							:style="`width: ${85 - (85 * time) / totalTime}%`"></div>
						<div v-else class="inner h-full bg-#71CB8A trs"
							:style="`width: ${85 - (85 * time) / totalGameTime}%`"></div>
					</div>
					<!-- TODO: more font -->
					<p class="text-22px lt-xxxl:text-16px w-35px lt-xxxl:w-20px text-end" v-if="question.type !== 'VALUE'">
						{{ totalTime - time }}
			...
			<div class="">
				<template v-if="question.meta" class="cardInside">
					<div class="flex flex-col mt-42px mb-38px lt-xxxl:mb-22px lt-sm:mt-0!">
						<p v-if="question.type !== 'VALUE'" class="font-500 text-18px lt-xxxl:text-14px text-#A5A9B5">
							{{ questionNumber }} вопрос из 10
						</p>
						<p v-else class="font-500 text-18px lt-xxxl:text-14px text-#A5A9B5">
							Суперигра
						</p>
						<p v-html="question.meta.title" class="text-40px lt-xxxl:text-28px font-700 leading-130% mt-16px">
						</p>
						<div class="relative">
							<img v-if="question.meta.image && question.type !== 'POSITION'"
								class="w-full h-256px lt-xxxl:h-200px object-cover q-shadow-sm mt-24px"
								:src="`https://api.рускод.рф/${question.meta.image}`" />
							<p class="absolute bottom-14px lt-sm:left-8px lt-sm:bottom-8px lt-sm:right-unset right-14px text-white text-14px lt-sm:text-8px leading-130%"
								v-if="question.meta.imageComment">
								{{ question.meta.imageComment }}
							</p>
						</div>
						<p v-if="question.meta.comment" v-html="question.meta.comment"
							class="font-500 text-26px lt-xxxl:text-22px leading-130% text-#1E2947 mt-42px lt-xxxl:mt-32px">
						</p>
						<p v-else class="font-500 text-26px lt-xxxl:text-22px leading-130% text-#1E2947 mt-28px lg:hidden">
							Выберите ответ
						</p>
					</div>
					<!-- TODO: переносы -->
					<div v-if="question.type === 'DEFAULT'" class="flex flex-wrap gap-16px"
						...
						<div v-for="answer in question.answers" @click="answerEmit(answer)"
							...
							<p v-html="answer.meta.title"
								class="text-20px lt-xxxl:text-14px leading-130% text-center py-22px p-18px"></p>
						</div>
					</div>
					<div v-else-if="question.type === 'VALUE'" class="">
						...
						<div class="flex items-start gap-x-9px mt-10px lt-sm:flex-wrap">
							<div class="flex flex-col items-center gap-y-8px"
								...
								<div class="h-66px lt-xxxl:h-55px w-66px lt-xxxl:w-55px flex items-center border-1px border-dashed border-#1E2947 text-#1E2947"
									...
									<input
										...
										:id="`input__${index}`" maxlength="1" v-model="letterModel[index]" placeholder="+"
						...
						<template v-if="question.meta && !question.meta.easy">
							<button v-if="totalGameTime - 10 > 0" @click="openRandomLetter"
					...
					<div v-else-if="question.type === 'POSITION'"
						...
						<div class="flex flex-wrap gap-16px lt-lg:flex-col lt-lg:w-[calc(100%/2-4px)]">
							<div v-for="(position, index) in question.meta.positions"
								class="bg-white q-answer q-shadow-sm w-[calc(100%/3-12px)]! lt-lg:w-full! lt-sm:max-h-73.3px flex items-center">
								<p
									class="text-20px lt-xxxl:text-18px lt-sm:text-14px leading-26px text-center py-22px p-18px lt-sm:px-12px my-auto w-full pointer-events-none">
									{{ position.title }}
								</p>
							</div>
						</div>
						<div class="flex flex-wrap gap-16px mt-18px lt-lg:mt-0 lt-lg:flex-col lt-lg:w-[calc(100%/2-4px)]">
							<div class="bg-white border-1px border-dashed border-#1E2947 h-74px min-h-74px lt-xxxl:max-h-56px w-[calc(100%/3-12px)]! lt-lg:w-full! cursor-grab! flex items-center justify-center"
								:class="[
								...
								]" v-for="zone in 3" @dragover.prevent="dragOver" @dragleave="dragLeave" @drop="stickerDrag(zone - 1, $event)"
								draggable="true" @dragstart="dragStart(positionAnswers[zone - 1], zone - 1, $event)"
						...
						<div class="flex flex-wrap gap-16px lt-lg:gap-6px mt-18px w-full">
							<div class="bg-white q-answer w-[calc(100%/3-12px)]! lt-lg:w-[calc(100%/2-3px)]! hover:scale-100! lt-lg:border-none cursor-grab"
								v-for="(answer, index) in question.answers" draggable="true"
								@dragend="dragEnd(answer.id, $event)" @dragstart="dragStart(answer, null, $event)"
								...
								<div class="flex items-center justify-center py-22px p-18px pointer-events-none">
									...
									<p class="text-20px lt-xxxl:text-18px leading-130% text-center ml-10px">{{
										answer.meta.title }}</p>
								</div>
							</div>
						</div>
			...
			<TheModalBottom v-model="showSuperGameEnd">
				<div class="absolute bottom-0 left-50% translate-x-[-50%] z-1010101001">
					<img class="absolute right-[-35%] lt-lg:right-[-15%] lt-lg:bottom-50% bottom-0 z-1 w-700px lt-lg:w-500px"
						src="/img/characters/pushkin-bg.png" />
					<div class="flex lt-sm:flex-col h-full lt-lg:max-h-166px! lt-sm:max-h-310px! lt-lg:min-h-166px items-center justify-between p-55px lt-lg:p-36px! gap-x-44px w-886px max-w-screen! lt-lg:w-672px min-h-223px relative z-2"
						:class="[{ 'bg-#D1EAD8': !superGameError }, { 'bg-#FFEAE7': superGameError }]">
						<div class="">
							<p class="text-#08882C font-500 text-36px lt-lg:text-30px leading-130%"
								:class="{ 'text-#C01E00': superGameError }">
								{{ !superGameError ? 'Красавчик!' : 'Правильный ответ:' }}
							</p>
							<p v-if="question.meta && question.meta.comment"
								class="text-26px leading-130% mt-18px lt-lg:text-18px lt-lg:mt-11px">
								{{ question.meta.comment }}
							</p>
						</div>
						<button @click="startResultCount"
			...
			<TheModalBottom v-model="showCountingModal">
				<div class="flex w-screen justify-center items-end">
					<img class="w-700px lt-lg:w-600px lt-lg:ml-[-130px] lt-xxxl:w-500px object-contain"
					...
					<div
						...
						<p class="font-500 text-36px lt-lg:text-30px lt-xxxl:text-28px lt-sm:text-20px leading-130%">
							Пару минут, мы считаем результаты<span class="lg:hidden">, а пока почитай, для чего мы сделали
								этот квиз</span>
						</p>
						<p
							class="text-26px lt-xxxl:text-22px lt-lg:text-18px lt-sm:text-14px leading-130% mt-32px lt-lg:mt-18px">
							{{ countingInfo.text }}</p>
